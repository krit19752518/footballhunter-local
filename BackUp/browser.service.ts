...
private static async executeTask(task: any) {
    ...
    try {
      ...
      for (let i = 0; i < await allBoxes.count(); i++) {
          ...
          const fullBoxText = await box.textContent({ timeout: 5000 }).catch(() => "") || "";
          const labelText = await box.locator('._bet-label_1ckm8_65').textContent({ timeout: 5000 }).catch(() => "") || ""; 
          
          // 🟢 ตรวจสอบความถูกต้องของประเภทกล่อง (HDP หรือ O/U)
          const isOverSignal = betSide.includes('สูง') || betSide.toLowerCase().includes('over');
          const isUnderSignal = betSide.includes('ต่ำ') || betSide.toLowerCase().includes('under');
          
          let isValidType = false;
          if (isOverSignal) {
              isValidType = fullBoxText.includes('สูง') || fullBoxText.includes('O');
          } else if (isUnderSignal) {
              isValidType = fullBoxText.includes('ต่ำ') || fullBoxText.includes('U');
          } else {
              // ถ้าเป็น แฮนดิแคป (HDP) ต้องไม่ใช่กล่อง สูง/ต่ำ
              isValidType = !fullBoxText.includes('สูง') && !fullBoxText.includes('ต่ำ') && !fullBoxText.includes('O') && !fullBoxText.includes('U');
          }

          if (isValidType && isLineMatchHelper(targetLine, labelText)) {
              await box.click({ force: true });
              found = true;
              break;
          }
      }
      if (!found) throw new Error(`ไม่พบราคาที่ต้องการ (Target: ${targetLine}, Side: ${betSide})`);
...
// 2. หาปุ่มลบ (ถังขยะ)
 ...
 static async findAndBet(leagueName: string, matchName: string, betSide: string, amount: number, targetLine?: string, isTest: boolean = false, taskId?: string) {
  ...
  public static async getActualBalance(): Promise<number | null> {
    if (!this.page) return this.lastBalance;

    const now = Date.now();
    // หน่วงเวลา 15 วินาทีเพื่อไม่ให้ดึงยอดเงินถี่เกินไปจนเว็บแบน
    if (now - this.lastBalanceTime < 15000) {
      return this.lastBalance;
    }

    try {
      const balanceData = await this.page.evaluate(async () => {
        try {
          // ใช้ Relative Path เพื่อไม่ให้ติดปัญหาเรื่อง Sub-domain เปลี่ยนไปมา
          const res = await fetch('/hall/api/gameCenter/gameApi/logout', {
            method: 'POST',
            headers: {
      ...
      if (balanceData && balanceData.code === 1 && balanceData.data) {
        const gameGold = balanceData.data.game_gold;
        if (gameGold !== undefined && gameGold !== null) {
          this.lastBalance = parseFloat(gameGold.toString());
          this.lastBalanceTime = now;
          this.smartLog(`💰 [BALANCE] อัปเดตยอดเงินคงเหลือจริง: ${this.lastBalance} บาท`);
        }
      }
    } catch (e: any) {
      this.smartLog(`⚠️ [BALANCE] ไม่สามารถดึงยอดเงินหน้าเว็บได้: ${e.message}`);
    }
